...
my $OUT_INDEX;
if (defined $file_out_index) {
	open $OUT_INDEX, '>' , $file_out_index;
}
			...
			for (my $i = 0; $i <= $#data; $i++) {
				if (($i != $target_column) && (! exists $delete_column{$i})) {
					...
					if (! exists $id{$col_id}) {
						...
						if (defined $file_out_index) {
							print {$OUT_INDEX} $col_id, " ", $id{$col_id}, " ", $data[$target_column], "\n";
						}
						$id_cntr++;
					}
